...
Tests should protect the formatter's behavioral boundaries without coupling every assertion to implementation inventory. This document explains where a test belongs, which global state it must isolate, and when compatibility or documentation work is required.
...
Prefer representative complete-output assertions when changing spacing, prefixes, or section order. Normalize only inherently variable values such as durations and temporary paths. Broad matchers that hide blank lines, source labels, resets, or ordering can allow scanability regressions.
