...
The formatter tracks the active example-group stack instead of splitting `Example#full_description`. Group and example descriptions can contain the configured separator, so paths are built from their individual descriptions. Empty group descriptions are omitted, and an empty example description falls back to its rerun argument. If the destination declares an external encoding that cannot represent the configured separator, path construction uses ` > `.

An example becomes current at `example_started` and remains current through its result notification. Output from example hooks is therefore attributed to that example. Group hooks run without a current example and use the active group path. The formatter tracks which groups have already become visible so context output emits a heading only when needed. Output outside any group uses suite labels and emits `RSpec suite` when it is the first visible report content.

Because attribution follows current formatter state rather than thread origin, same-process parallel examples cannot be attributed reliably. Threads are synchronized, but a late write belongs to the state active when the write is handled, not necessarily to the example whose code created the thread.
...
Expected-pending exceptions can include RSpec's failure presentation. An explicitly configured formatter `pending_failure_output` controls the formatter's filtering; otherwise the formatter uses RSpec's setting when that API exists. Full mode keeps the lines returned by RSpec, no-backtrace mode removes lines whose stripped content starts with `# `, and skip mode does not request expected-failure details. This setting does not reconfigure RSpec's presenter, so formatter `:full` cannot restore backtrace lines that RSpec has already omitted.

For failures, the formatter asks the notification for `fully_formatted_lines` and supplies its own colorizer when supported. This preserves RSpec's source extraction, filtered backtraces, aggregate failures, shared-example traces, causes, and extra failure lines. Older notification shapes fall back to available message lines or the presenter call without a colorizer. On the primary presenter path, the formatter removes a leading full description only when it matches the example description exactly after stripping ANSI.
...
POSIX arguments use `Shellwords.escape`. Windows arguments pass through `WindowsCommandLine`, which rejects NUL and newlines and protects both ordinary command-line quoting and `cmd.exe` metacharacters. Rerun arguments use two `cmd.exe` escaping layers because RubyGems invokes `rspec` through a batch launcher that forwards `%*`.
...
Profiles are rendered only when RSpec requests them. The formatter accepts profile notification shapes that may omit slow-example or slow-group fields, and it emits a visible no-examples line when neither collection contains entries.
