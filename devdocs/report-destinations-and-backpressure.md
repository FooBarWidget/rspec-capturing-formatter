...
Formatter-generated output uses `write`, even when the destination also supports `write_nonblock`, so an RSpec notification is not failed by a nonblocking destination that requires a retry. Report bytes generated by an application `write_nonblock` or `syswrite` use the destination's `write_nonblock` method when available. A numeric return smaller than the pending byte count advances only by that amount and immediately attempts the remainder. A `nil` return is treated as accepting the complete pending chunk, matching common writer objects that do not return a byte count.
