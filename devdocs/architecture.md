...
The capture manager is process-global. Each formatter instance has its own renderer. The manager routes captured writes only to the formatter that owns the active lease, and can activate only one different formatter destination at a time.
